    ...
    <div class="side-nav" @mouseenter="isFade = false" :class="{ 'is-fade': isFade }" :style="navStyle">
        ...
        <ul>
            <li class="nav-item" v-for="(item, key) in data" :key="key">
                ...
                <router-link v-if="item.path" active-class="active" :to="to(item.path)" exact v-text="item.title || item.name">
                </router-link>
                <ul class="pure-menu-list sub-nav" v-if="item.children">
                    <li class="nav-item" v-for="(navItem, key) in item.children" :key="key">
                        <router-link class="" active-class="active" :to="to(navItem.path)" exact v-text="navItem.title || navItem.name">
                        </router-link>
                    </li>
                </ul>
                    ...
                    <div class="nav-group" v-for="(group, key) in item.groups" :key="key">
                        ...
                        <ul class="pure-menu-list">
                            <li class="nav-item" v-for="(navItem, key) in group.list" v-show="!navItem.disabled" :key="key">
                                <router-link active-class="active" :to="to(navItem.path)" exact v-text="navItem.title"></router-link>
                            </li>
                        </ul>
                    </div>
                </template>
...
export default {
    ...
    computed: {
        to: {
            get: function () {
                return (path) => {
                    return this && this.lang ? "/" + this.lang + path : "/" + path;
                };
            },
        },
        navStyle() {
